...
# expect: programs created…, skills created…, readme updated, kernel created (~20 MB)
```
...
- First sync of a joined space (incl. the kernel file) can lag the
  join; serve waits up to 180 s for membership, and a kernel fetch
  failing right after is the documented hard error — rerun serve.
