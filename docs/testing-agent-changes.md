...
| | test rig | real bao |
|---|---|---|
| any server | `127.0.0.1:7009`, data dir `~/any/any-test-7009` | `127.0.0.1:7001` |
| serve control port | `7011` | `7010` |
...
| traces | `traces-test/` | `traces/` |
| config | `anybao.test.toml` (gitignored — holds the account + space ids) | `anybao.toml` |

The account is persistent — data dir + mnemonic backup live in
`~/any/any-test-7009/`; never recreate it, restart the server and it
...
```fish
curl -s http://127.0.0.1:7009/v1/health   # test server up? (account non-empty = ours)
...
cd ~/any/anybao
./runtime/target/release/anyrt deploy --source . --target agent --config-file anybao.test.toml

# 3. the agent
ANTHROPIC_API_KEY=(cat ../anthropic.key) ./runtime/target/release/anyrt serve --config-file anybao.test.toml

# 4. the UI, pointed at the rig (5175 so the real one keeps 5173)
cd ~/any/any-ui && VITE_API_TARGET=http://127.0.0.1:7009 pnpm dev --port 5175
```
