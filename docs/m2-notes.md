...
- [ ] Digest v1 (ADR-005 §4): budget-aware inline/stub (tokens),
...
- [ ] Loop v1 (ADR-005 §2/§3): ceilings→wrap-up turn, mailbox
      (inject/break soft+hard), turn-record shaping (deferred write
      until anyclient), progress bubbles.
- [ ] llm effect + adapters (ADR-005 §1): neutral message model;
